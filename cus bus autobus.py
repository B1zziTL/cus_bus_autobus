...
def data(event): #funkcia na vykreslenie dat z textu
    ...
    if obsadenost <= 50 and obsadenost != 0:
        canvas.create_rectangle(250,x1,400,x1+20)
        canvas.create_rectangle(251,x1+1,251+(obsadenost*3),x1+20,fill='green',outline='')
    if obsadenost >= 50:
        canvas.create_rectangle(250,x1,400+(obsadenost*3-149),x1+20)
        canvas.create_rectangle(251,x1+1,251+(obsadenost*3),x1+20,fill='red',outline='')
    x1 += 30
    i += 1
